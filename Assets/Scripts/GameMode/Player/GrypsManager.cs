...
public class GrypsManager : MonoBehaviour
{
    ...
    [Header("stageCtrl")] public StageCtrl stageCtrl;
    ...
    void AllGetComponent()
    {
        ...
        memoryGageMg = stageCtrl.memoryGageMg.GetComponent<MemoryGageManager>();
        jetMemoryMg = stageCtrl.jetMemoryMg.GetComponent<JetMemoryManager>();
    ...
    private void Landing()
    {
        ...
        if ((int)stageCtrl.state == 2 && memoryGageMg.memoryGage >= 0)
        {
            stageCtrl.Regeneration();
    ...
    public void EnterWarp(Transform transferDestination)
    {
        ...
        key = (int)transferDestination.localScale.x;
    }

    public void ExitWarp(int forceNum)
    {
        rb.velocity = new Vector2(3f, 0f);
        rb.AddForce(key * transform.right * dashLevel[forceNum]);
    }
    ...
    public void SomerSault(float angularChangeInDegrees)
    {
        if (!isSomersault)
        {
            ...
            rb.AddTorque(key * impulse, ForceMode2D.Impulse);
            isSomersault = true;
